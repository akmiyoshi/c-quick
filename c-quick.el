...
;(setq explicit-shell-file-name (format "%s/.software/_busybox/current/bash.exe" (getenv "USERPROFILE")))
(global-set-key (kbd "<f10>")
                (lambda ()
                  ...
                  (if (null buffer-file-name) (ding)
                      (let* ((shell-file-name
                              ;(format "%s/.software/_busybox/current/bash.exe" (getenv "USERPROFILE"))
                              (format "%s/.software/_msys2/current/usr/bin/bash.exe" (getenv "USERPROFILE"))
                              )
